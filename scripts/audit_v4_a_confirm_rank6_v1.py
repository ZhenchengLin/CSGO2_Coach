#!/usr/bin/env python3
"""Read-only Rank 6 V4-A technical audit and one-time structured evidence.
...
AUDIT_VERSION = 'V4_A_CONFIRM_RANK6_TECHNICAL_EVIDENCE_V1'
RANK = 6
MATCH_ID = '2397834'
DEMO_SHA = '7dcfab14a82dd97e9b0ebbedd346c305320aecbef1fee127553a2c4187c38f2a'
ARCHIVE_SHA = '56da0a755664fa58d8d96caf99140e7476fb84cfe4a58012ed5865927bb17d6a'
EXTRACTION_SHA = '7e6089ba2b397c7b933565dc30439afc8e8b4ad12511ed4a3d33703fe96ee149'
RANK2_SHA = '874496b96943e1cdac691494c9821fe5d4389ba4e173df0f048c30254aca0ead'
SOURCE_SHA = 'a8dc41878f4b9c3cea7911f077eb9ee2d98b9b25b99cbc2a8e1ae09349863bea'
MEMBER = 'lavked-vs-navi-junior-m2-mirage.dem'
STAGING = ROOT / 'data/raw/v4_a_confirm_download_staging/rank_06_2397834'
DEMO = STAGING / MEMBER
EXTRACTION = ROOT / 'docs/v4_a_confirm_intake_audits/rank_06_selected_demo_extraction_v1.json'
RANK2 = ROOT / 'docs/v4_a_confirm_intake_audits/rank_02_technical_eligibility_v1.json'
SOURCE = ROOT / 'docs/v4_a_confirm_intake_audits/rank_06_match_page_source_v1.html'
OUTPUT = ROOT / 'docs/v4_a_confirm_intake_audits/rank_06_data_integrity_v1.json'
...
SOURCE_FILES = (
    ...
    'docs/v4_a_confirm_intake_events_v1/rank_06_2397834_archive_acquired.json',
    'docs/v4_a_confirm_intake_audits/rank_06_selected_demo_extraction_v1.json',
    'docs/v4_a_confirm_intake_audits/rank_06_match_page_source_v1.html',
    ...
    'scripts/audit_v4_a_confirm_rank6_v1.py',
    'tests/test_v4_a_confirm_rank6_contract_v1.py',
...
def require(value, message):
    if not value:
        raise RuntimeError('RANK6_AUDIT_STOP: ' + message)
...
def verify_inputs():
    ...
    require(digest(EXTRACTION) == EXTRACTION_SHA and digest(SOURCE) == SOURCE_SHA, 'Pinned Rank 6 source or extraction evidence changed')
    ...
    require(
        ...
        'Rank 6 duplicates the earlier eligible Rank 3 Demo',
    )
    ...
    require(rows[5]['source_match_id'] == MATCH_ID and initial[5]['source_match_id'] == MATCH_ID, 'Rank 6 queue identity mismatch')
    event = read_event(initial[5])  # full archive hash and acquisition record verification
    require(event and event['archive_sha256'] == ARCHIVE_SHA and event['status'] == 'ARCHIVE_ACQUIRED', 'Rank 6 Archive Event mismatch')
    ...
    require(digest(DEMO) == DEMO_SHA and DEMO.stat().st_size == 316833486, 'Actual selected Demo bytes changed')
    ...
    require(maps == ['Dust2', 'Mirage', 'Ancient'] and 'Match over' in parsed.get_text(' ', strip=True), 'Match source map/completion differs')
    ...
    require(date == rows[5]['match_date'] == '2026-09-21', 'Match date differs from frozen queue')
    require(shutil.disk_usage(ROOT).free >= 12 * 1024**3, 'STORAGE_BLOCK: below frozen 12 GiB floor')
    print('Rank 1/2 decisions, frozen Rank 6 identity, acquisition and extraction SHA256: VERIFIED')
...
def audit():
    ...
    print('RANK6_DATA_INTEGRITY_AUDIT_PASSED')
...
def main():
    ...
    if args.check:
        ...
        print('RANK6_DATA_INTEGRITY_EVIDENCE_VERIFIED | no Demo reparse | no model scoring')
    elif OUTPUT.exists():
        require(OUTPUT.is_file() and not OUTPUT.is_symlink(), 'Existing evidence path invalid')
        check_saved(json.loads(OUTPUT.read_text()))
        print('RANK6_DATA_INTEGRITY_EVIDENCE_VERIFIED | existing record preserved | no reparse')
    else:
        audit()
