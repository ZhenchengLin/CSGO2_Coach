#!/usr/bin/env python3
"""Read-only Rank 3 V4-A technical audit and one-time structured evidence.
...
AUDIT_VERSION = 'V4_A_CONFIRM_RANK3_TECHNICAL_EVIDENCE_V1'
RANK = 3
MATCH_ID = '2397693'
DEMO_SHA = 'b6e4f8e7170fd3be55f067661e33e8c28bc4ae59792242cf4580ac22abdb16f8'
ARCHIVE_SHA = '48d3a9be8776940dadaa19b450eb887e5ed6fdf3467d1e57196011a0eda73a74'
EXTRACTION_SHA = '109995dfc5624c4b250607e802378d36079c8be20a6348fb1993b81626a25c57'
RANK2_SHA = '874496b96943e1cdac691494c9821fe5d4389ba4e173df0f048c30254aca0ead'
SOURCE_SHA = '5e481dc8f7687ccceff9773e52c1222cc1558fa5a420da3eb6c7282bcb0ef5d4'
MEMBER = 'leo-vs-g2-ares-m1-mirage.dem'
STAGING = ROOT / 'data/raw/v4_a_confirm_download_staging/rank_03_2397693'
DEMO = STAGING / MEMBER
EXTRACTION = ROOT / 'docs/v4_a_confirm_intake_audits/rank_03_selected_demo_extraction_v1.json'
RANK2 = ROOT / 'docs/v4_a_confirm_intake_audits/rank_02_technical_eligibility_v1.json'
SOURCE = ROOT / 'docs/v4_a_confirm_intake_audits/rank_03_match_page_source_v1.html'
OUTPUT = ROOT / 'docs/v4_a_confirm_intake_audits/rank_03_data_integrity_v1.json'
...
SOURCE_FILES = (
    ...
    'docs/v4_a_confirm_intake_events_v1/rank_03_2397693_archive_acquired.json',
    'docs/v4_a_confirm_intake_audits/rank_03_selected_demo_extraction_v1.json',
    'docs/v4_a_confirm_intake_audits/rank_03_match_page_source_v1.html',
    ...
    'scripts/audit_v4_a_confirm_rank3_v1.py',
...
def require(value, message):
    if not value:
        raise RuntimeError('RANK3_AUDIT_STOP: ' + message)
...
def verify_inputs():
    ...
    require(digest(EXTRACTION) == EXTRACTION_SHA and digest(SOURCE) == SOURCE_SHA, 'Pinned Rank 3 source or extraction evidence changed')
    ...
    require(rows[2]['source_match_id'] == MATCH_ID and initial[2]['source_match_id'] == MATCH_ID, 'Rank 3 queue identity mismatch')
    event = read_event(initial[2])  # full archive hash and acquisition record verification
    require(event and event['archive_sha256'] == ARCHIVE_SHA and event['status'] == 'ARCHIVE_ACQUIRED', 'Rank 3 Archive Event mismatch')
    evidence = json.loads(EXTRACTION.read_text())
    require(evidence['candidate_rank'] == 3 and evidence['source_match_id'] == MATCH_ID and evidence['extracted_demo_sha256'] == DEMO_SHA, 'Selected-member evidence mismatch')
    ...
    require(digest(DEMO) == DEMO_SHA and DEMO.stat().st_size == 297602564, 'Actual selected Demo bytes changed')
    ...
    require(maps == ['Mirage', 'Cache', 'Ancient'] and 'Match over' in parsed.get_text(' ', strip=True), 'Match source map/completion differs')
    ...
    require(date == rows[2]['match_date'] == '2026-09-21', 'Match date differs from frozen queue')
    require(shutil.disk_usage(ROOT).free >= 12 * 1024**3, 'STORAGE_BLOCK: below frozen 12 GiB floor')
    print('Rank 1/2 decisions, frozen Rank 3 identity, acquisition and extraction SHA256: VERIFIED')
...
def audit():
    ...
    print('RANK3_DATA_INTEGRITY_AUDIT_PASSED')
...
def main():
    ...
    if args.check:
        ...
        print('RANK3_DATA_INTEGRITY_EVIDENCE_VERIFIED | no Demo reparse | no model scoring')
    elif OUTPUT.exists():
        require(OUTPUT.is_file() and not OUTPUT.is_symlink(), 'Existing evidence path invalid')
        check_saved(json.loads(OUTPUT.read_text()))
        print('RANK3_DATA_INTEGRITY_EVIDENCE_VERIFIED | existing record preserved | no reparse')
    else:
        audit()
...
if __name__ == '__main__':
    try:
        ...
        print('RANK 3 AUDIT STOP:', repr(exc), file=sys.stderr)
        raise SystemExit(1)
